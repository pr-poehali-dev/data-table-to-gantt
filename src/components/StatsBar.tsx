...
import { PROJECTS, hoursToDays } from '@/data/mockData';
...
export default function StatsBar({ tasks, activeProjectId }: StatsBarProps) {
  ...
  const stats = [
    ...
    { label: 'Заблокировано',     value: blocked,                        icon: 'Lock',        color: 'text-purple-600' },
  ...
  return (
    <div className="flex items-center gap-0 border-b border-border bg-card overflow-x-auto scrollbar-thin">
      {/* Цветовые метки проектов при виде "все" */}
      {activeProjectId === 'all' && (
        <div className="flex items-center gap-2 px-4 py-3 border-r border-border flex-shrink-0">
          {PROJECTS.map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-1" title={p.name}>
                ...
                <span className="text-[10px] mono text-muted-foreground">{p.code}: {cnt}</span>
              </div>
            );
          })}
        </div>
      ...
      {stats.map((s, i) => (
        <div key={s.label} className={`flex items-center gap-2.5 px-4 py-3 ${i < stats.length - 1 ? 'border-r border-border' : ''} whitespace-nowrap flex-shrink-0`}>
      ...
      {total > 0 && (
        <>
          ...
          <div className="px-4 py-3 flex-shrink-0 border-l border-border">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium mono text-foreground">{total > 0 ? Math.round((done / total) * 100) : 0}%</span>
            </div>
          </div>
        </>
      )}
